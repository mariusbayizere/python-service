...
# CHANGE 1 of 2: added send_from_directory to this import line
...
# ============================================================
# DRUNK DRIVING BEHAVIOUR DETECTOR
# Detects drunk driving by analysing PATTERN of violations
# If drowsy + distracted fire repeatedly in same window -> DRUNK_DRIVING
# This works because drunk drivers show MULTIPLE impairment signs together
# ============================================================

class DrunkDrivingDetector:
    """
    Detects drunk driving from behaviour patterns.
    A drunk driver shows drowsiness AND distraction TOGETHER repeatedly.
    A tired-but-sober driver usually shows one or the other.
    """
    def __init__(self, window_seconds=60, threshold=2):
        self.window_seconds = window_seconds  # 60 second observation window
        self.threshold      = threshold       # how many times each sign must appear
        self.drowsy_times   = []              # timestamps of drowsy detections
        self.distract_times = []              # timestamps of distraction detections
        self.drunk_fired_at = 0              # last time drunk_driving was fired

    def record_event(self, alert_name):
        """Call this every time drowsy or distracted fires."""
        now = time.time()
    ...
    def is_drunk(self):
        """
        Returns True if behaviour pattern matches drunk driving.
        Condition: drowsy fired 2+ times AND distracted fired 2+ times
        within the same 60-second window.
        """
        # Cooldown — don't fire drunk_driving more than once per 5 minutes
        if time.time() - self.drunk_fired_at < 300:
            return False
        if len(self.drowsy_times) >= self.threshold and len(self.distract_times) >= self.threshold:
            self.drunk_fired_at = time.time()
            print("DRUNK DRIVING PATTERN DETECTED:")
            print("  Drowsy events in last 60s: " + str(len(self.drowsy_times)))
            print("  Distraction events in last 60s: " + str(len(self.distract_times)))
            return True
        return False
    ...
    def reset(self):
        self.drowsy_times   = []
        self.distract_times = []
        self.drunk_fired_at = 0

drunk_detector = DrunkDrivingDetector(window_seconds=60, threshold=2)
...
def _fire_drunk_driving(frame):
    """Fire a DRUNK_DRIVING violation — highest severity."""
    ...
    data = {
        ...
        'severity':      'CRITICAL',
        'description':   (
            driver_name + " is showing multiple simultaneous impairment signs "
            "(repeated drowsiness + distraction) consistent with drunk driving "
            "while on " + trip_name + " in " + vehicle_name + ". "
    ...
    # Play urgent audio
    if TTS_AVAILABLE and monitoring_active:
        def _say():
            with _audio_lock:
                if monitoring_active:
                    _tts_engine.say("URGENT WARNING! Driver appears to be under the influence of alcohol. Contact authorities immediately.")
                    _tts_engine.runAndWait()
    ...
    if send_violation_to_java(data):
        ...
        send_sms_alert('drowsy', current_session['driver_id'], current_session['trip_id'])
...
def handle_violation(frame, alert_name):
    ...
    # Feed event into drunk driving detector
    drunk_detector.record_event(alert_name)
    if drunk_detector.is_drunk():
        print("ESCALATING TO DRUNK_DRIVING")
        _fire_drunk_driving(frame)
...
# CHANGE 2 of 2: new route added here — serves individual screenshot image files
# React calls: GET http://localhost:5000/api/screenshots/distracted_20260413_072649.jpg
@app.route('/api/screenshots/<path:filename>', methods=['GET'])
def serve_screenshot(filename):
    try:
        print("DEBUG serve_screenshot: Request for file: " + filename)
        print("DEBUG serve_screenshot: Looking in: " + SCREENSHOTS_DIR)

        filepath = os.path.join(SCREENSHOTS_DIR, filename)

        if not os.path.exists(filepath):
            print("DEBUG serve_screenshot: FILE NOT FOUND: " + filepath)
            if os.path.exists(SCREENSHOTS_DIR):
                files = os.listdir(SCREENSHOTS_DIR)
                print("DEBUG serve_screenshot: Directory contains " + str(len(files)) + " files")
                matches = [f for f in files if filename.lower() in f.lower()]
                if matches:
                    print("DEBUG serve_screenshot: Similar files found: " + str(matches))
            return jsonify({'error': 'File not found: ' + filename}), 404

        size = os.path.getsize(filepath)
        print("DEBUG serve_screenshot: Serving " + filename + " (" + str(size) + " bytes)")

        response = send_from_directory(SCREENSHOTS_DIR, filename, mimetype='image/jpeg')
        response.headers['Access-Control-Allow-Origin'] = '*'
        response.headers['Cache-Control'] = 'public, max-age=3600'
        return response

    except Exception as e:
        print("DEBUG serve_screenshot: ERROR — " + str(e))
        import traceback
        traceback.print_exc()
        return jsonify({'error': str(e)}), 500
...
def test_yolo():
    """
    Test YOLO detection. Hold bottle/food/phone in front of camera first.
    GET http://localhost:5000/api/test-yolo
    """
    # First show the actual class names from the file
    actual_classes = []
    try:
        with open(YOLO_NAMES) as f:
            actual_classes = [l.strip() for l in f.readlines()]
    except Exception as e:
        return jsonify({'error': 'Cannot read coco.names: ' + str(e)}), 500

    # Find all food/drink/phone related classes in the actual file
    food_drink_phone = []
    keywords = ['bottle','cup','bowl','wine','glass','banana','apple','orange',
                'sandwich','pizza','donut','cake','fork','knife','spoon','hot','dog',
                'phone','cell','remote','mouse','broccoli','carrot','vase','can','drink']
    for i, cls in enumerate(actual_classes):
        for kw in keywords:
            if kw in cls.lower():
                food_drink_phone.append({'index': i, 'class': cls})
                break
    ...
    if yolo_net is None:
        return jsonify({
            ...
            'food_drink_phone_in_file': food_drink_phone,
        }), 500
    ...
    # Run with very low threshold 0.1 to catch everything
    all_detected = []
    try:
        ...
        for output in outputs:
            for det in output:
                ...
                if confidence < 0.1:
                    continue
                label = actual_classes[cid] if cid < len(actual_classes) else "cls_" + str(cid)
                all_detected.append({'label': label, 'confidence': round(confidence, 3), 'class_id': cid})
        all_detected.sort(key=lambda x: x['confidence'], reverse=True)
    except Exception as e:
        return jsonify({'error': 'YOLO failed: ' + str(e)}), 500

    # Check against our sets using ACTUAL file class names
    drinking_found = [d for d in all_detected if d['label'].lower() in DRINKING_OBJECTS and d['confidence'] >= 0.3]
    eating_found   = [d for d in all_detected if d['label'].lower() in EATING_OBJECTS   and d['confidence'] >= 0.3]
    phone_found    = [d for d in all_detected if d['label'].lower() in PHONE_OBJECTS     and d['confidence'] >= 0.3]

    return jsonify({
        'status':                  'YOLO running',
        'actual_classes_in_file':  len(actual_classes),
        'food_drink_phone_classes': food_drink_phone,
        'all_detections_above_10pct': all_detected[:20],
        'drinking_found':          drinking_found,
        'eating_found':            eating_found,
        'phone_found':             phone_found,
        'test_image_saved':        test_path,
        'instructions': [
            "1. Hold a WATER BOTTLE at face level, 30cm from camera",
            "2. Make sure room is well lit",
            "3. Call this endpoint again",
            "4. Look for bottle in all_detections_above_10pct"
        ],
        'important': "If bottle still not detected, your coco.names may use different class names. Check food_drink_phone_classes field."
    })
...
if __name__ == '__main__':
    ...
    print("  GET  /api/screenshots/<filename>")
