...
EATING_OBJECTS   = {"sandwich", "hot dog", "pizza", "donut", "cake",
                    "apple", "banana", "orange", "fork", "knife", "spoon"}
DRINKING_OBJECTS = {"bottle", "wine glass", "cup", "bowl"}

def detect_smoking_eating(frame):
    result = {'smoking': False, 'eating': False, 'drinking': False}
    if yolo_net is None:
        return result
    try:
        ...
        for output in outputs:
            for det in output:
                ...
                if float(scores[cid]) < 0.5:
                    continue
                label = yolo_classes[cid].lower() if cid < len(yolo_classes) else ""
                if label in SMOKING_OBJECTS:
                    print("DEBUG YOLO: smoking object detected — " + label)
                    result['smoking'] = True
                if label in EATING_OBJECTS:
                    print("DEBUG YOLO: eating object detected — " + label)
                    result['eating'] = True
                if label in DRINKING_OBJECTS:
                    print("DEBUG YOLO: drinking object detected — " + label)
    ...
    while monitoring_active:
        ...
        if frame_count % 30 == 0:
            ...
            if frame_count % yolo_interval == 0 and yolo_net is not None:
                for behaviour, detected in detect_smoking_eating(frame).items():
